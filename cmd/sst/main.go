...
var subCmds = map[string]subCmd{
	"info": {description: "Print hierarchical SST information", f: subCmdInfo},
...
func subCmdInfo(args []string) error {
	...
	allInfo := make(map[utils.ID]*sst.PackageInfo, len(pkgs))
...
// nolint:errcheck
func printInfoTable(allInfo map[utils.ID]*sst.PackageInfo) {
	...
	w := newTable()
	fmt.Fprintln(w, "  PKG\tPUNIT\tSUPPORTED\tLOCKED\tVERSION\tCURRENT LEVEL\tMAX LEVEL\tCPUS")
	for _, pkgID := range pkgIDs {
		for _, punitID := range slices.Sorted(maps.Keys(allInfo[pkgID].Punits)) {
			pu := allInfo[pkgID].Punits[punitID]
			fmt.Fprintf(w, "  %d\t%d\t%v\t%v\t%d\t%d\t%d\t%s\n",
				pkgID, punitID,
