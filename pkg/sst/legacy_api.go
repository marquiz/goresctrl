...
//
// Deprecated: Use Init and Package.GetInfo instead.
...
//
// Deprecated: Use ClosConfig and ClosInfo instead.
...
//
// Deprecated: CPU associations are now in ClosInfo.CPUs.
...
func GetPackageInfo(pkgs ...int) (map[int]*SstPackageInfo, error) {
	...
	for _, i := range pkglist {
		pi, err := legacyBackend.getPackageInfo(packages[i])
		if err != nil {
			return nil, err
		}
		info, err := infoFromPackageInfo(packages[i], pi)
		if err != nil {
...
// infoFromPackageInfo converts a hierarchical PackageInfo to a flat SstPackageInfo.
// Feature state is taken from the lowest id punit.
func infoFromPackageInfo(pkg *cpuPackageInfo, pi *PackageInfo) (*SstPackageInfo, error) {
	if len(pi.Punits) == 0 {
