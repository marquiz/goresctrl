...
func CPSetState(socketID, punitID uint8, enable bool, priority int) error {
...
func CPSetPriorityType(socketID, punitID uint8, priority int) error {
	...
	cpState.Get_set = 1
	cpState.Priority_type = uint8(priority)
	if err := isst.Ioctl(isst.ISST_IF_CORE_POWER_STATE, uintptr(unsafe.Pointer(&cpState))); err != nil {
		return fmt.Errorf("ISST_IF_CORE_POWER_STATE set for socket %d punit %d: %w", socketID, punitID, err)
	}
	return nil
...
func CPGetPriorityType(socketID, punitID uint8) (int, error) {
	state, err := CPGetState(socketID, punitID)
	if err != nil {
		return 0, err
	}
	return int(state.Priority_type), nil
}
...
// ClosSetParam writes CLOS parameters for one CLOS on one punit
// interface. Frequency values are in MHz.
// The epp and desiredFreq parameters are not supported on TPMI; non-zero values
// produce a warning and are ignored.
func ClosSetParam(socketID, punitID, clos uint8, epp, proportional, minFreq, maxFreq, desiredFreq int) error {
	param := isst.ClosParam{
		Get_set:         1,
		Socket_id:       socketID,
		Power_domain_id: punitID,
		Clos:            clos,
		Prop_prio:       uint8(proportional),
		Min_freq_mhz:    uint16(minFreq),
		Max_freq_mhz:    uint16(maxFreq),
	}
	if epp != 0 {
		log.Warn("ignoring EPP value, not supported on this platform", "clos", clos, "punit", punitID, "epp", epp)
	}
	if desiredFreq != 0 {
		log.Warn("ignoring DesiredFreq value, not supported on this platform", "clos", clos, "punit", punitID, "desiredFreq", desiredFreq)
	}
...
func ClosReset(socketID, punitID uint8) error {
	for clos := 0; clos < numClos; clos++ {
		if err := ClosSetParam(socketID, punitID, uint8(clos), 0, 0, 0, 25500, 0); err != nil {
			return err
		}
	}
	return nil
