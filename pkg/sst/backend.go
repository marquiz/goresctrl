...
func (h *backend) getPackageInfo(pkg *cpuPackageInfo) (*PackageInfo, error) {
	if h.isTPMIPlatform() {
...
func (h *backend) cpSetPriorityType(pkg *cpuPackageInfo, pu *punitInfo, priority CPPriorityType) error {
	if h.isTPMIPlatform() {
		return tpmi.CPSetPriorityType(pkg.id, pu.id, int(priority))
	}
...
func (h *backend) closConfigure(pkg *cpuPackageInfo, pu *punitInfo, clos int, config *ClosConfig) error {
	...
	if h.isTPMIPlatform() {
		...
		return tpmi.ClosSetParam(pkg.id, pu.id, closID, config.epp, config.ProportionalPriority, config.MinFreq, config.MaxFreq, config.desiredFreq)
	}
...
func getPackageInfoTPMI(pkg *cpuPackageInfo) (*PackageInfo, error) {
	socketID := pkg.id
	punits := pkg.punits
	pkgPunit := punits[0]

	// Read package-level PP info from first punit
	perfInfo, err := tpmi.PPGetPerfLevels(socketID, pkgPunit.id)
	if err != nil {
		return nil, err
	}
	// Check all punits for consistency
	for _, pu := range punits[1:] {
		pi, err := tpmi.PPGetPerfLevels(socketID, pu.id)
		if err != nil {
			return nil, err
		}
		pi.Socket_id = 0
		...
		if ref != pi {
			sstlog.Debug("ISST_IF_PERF_LEVELS inconsistency detected", "socket", socketID, "punitRef", pkgPunit.id, "punit", pu.id)
		}
	}

	info := &PackageInfo{
		ID:     utils.ID(socketID),
		Punits: make(map[utils.ID]*PunitInfo),
	}
...
func getPackageInfoMbox(pkg *cpuPackageInfo) (*PackageInfo, error) {
	...
	closInfos := make([]ClosInfo, numClos)
